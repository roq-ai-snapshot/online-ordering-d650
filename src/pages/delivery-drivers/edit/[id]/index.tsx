...
import { useRoqClient, useDeliveryDriverFindFirst } from 'lib/roq';
...
function DeliveryDriverEditPage() {
  ...
  const { data, error, isLoading, mutate } = useDeliveryDriverFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: DeliveryDriverInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.delivery_driver.update({
        data: values as RoqTypes.delivery_driver,
        where: {
          id,
        },
      });
