...
import { useRoqClient, useOrderItemFindManyWithCount } from 'lib/roq';
...
export function OrderItemListPage(props: OrderItemListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useOrderItemFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.order_item.delete({ where: { id } });
      await mutate();
    } catch (error) {
